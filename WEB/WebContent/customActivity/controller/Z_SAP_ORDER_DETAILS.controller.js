sap.ui.define(
	...
	function(
		...
		return Controller.extend("customActivity.controller.Z_SAP_ORDER_DETAILS", {
		onInit: function() {
	...
	getParentNode : function(){
		...
		var planthierarchy = this.interfaces.interfacesGetPHNodesForUserInput(this.appData.client, this.appData.plant);
		var parentNode = planthierarchy.nodes.results.find((item) => {
		return item.nodeID == this.appData.node.nodeID;
		},this).parentNodeID;
		var parentLineID = planthierarchy.nodes.results.find((item) => {
		return item.nodeID == parentNode;
		},this)?.lineNodeID;

		if(parentLineID){
			this.appData.node.parentNodeID = parentLineID;
		}
		else{
			delete this.appData.node.parentNodeID;
		}
...
ResourceBundle: function(iv_id) {
	...
	onPressDetails:function(oEvent){
        var details = [];
        var count, intervalcapacityLength;
        var capacityString="";
	...
	bindOrderDetailsToTable : function(){ 
		try{
        var currentNode = this.appData.node.parentNodeID || this.appData.node.nodeID;
		this.orderDetails = this.interfaces.interfacesGetOrderDetailsAndProductionRunIntervals(currentNode,this.appData.selected.order.orderNo,this.appData.selected.operationNo,this.appData.selected.releasedID.toString());
	...
	*/
	...
	openEnterDateTimeDialog : function(timeText,date){
		...
		if(this.oEnterTimeDialog == undefined){
			this.oEnterTimeDialog = sap.ui.xmlfragment("enterDateTimeFragment","sap.oee.ui.fragments.enterDateTime",this);
		...
		if(valueStateDate == "None" && valueStateTime == "None"){	
			try{
				...
				if(date && time && date !="" && time !=""){
					combinedDate = sap.oee.ui.Utils.createTimestampFromDateTime(date, time);
    ...
    onPressReprocessing: function() {
        var client = this.appData.PODs.client;
        var plant =this.appData.PODs.plant;
        var nodeID =this.appData.node.nodeID;
        var releasedID = this.appData.selected.releasedID;
         var response = TransactionCaller.async(
                        "ItelliMES/UI/Order_Details/T_CompletedToHold",
                        { I_CLIENT: client, I_PLANT: plant, I_NODEID: nodeID, I_RELEASEDDEMANDID: releasedID },
                        "O_JSON",
                        this.onPressReprocessingCB,
                        this
                    );
                },
                ...
                onPressReprocessingCB: function(iv_data, iv_scope) {
                    if(iv_data[0].ResponseStatus.header.executionStatus=="FAILED"){
                        MessageToast.show(iv_data[0].ResponseStatus.header.errorMessages.errorMessage);
		...
		var oBundle = iv_scope
						...
						.getModel("i18n")
						.getResourceBundle();
					var sMsg = oBundle.getText("KardemirOrderSuccessful");
