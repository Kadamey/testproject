	...
	function (
		...
		return Controller.extend("customOrderScreen.controller.tamponLoadScreen", {
			onInit: function () {
				...
				this.draggedTable;
				this.droppedTable;
				this.idTamponListTable;
				this.firstLayerTable;
				this.secondLayerTable;
				this.thirdLayerTable;
				this.fourthLayerTable;
			...
			_onRoute: function (oEvent) {
				...
				// var KMSQC = oEvent.getParameter("arguments").KMSQC;
...
if(iv_data[1] == "E"){
		iv_scope.getView().byId("idTamponListTable").setModel(null);

			//		iv_scope.getView().byId("idTamponListTable").getModel().refresh();
		iv_scope.getView().byId("firstLayerTable").setModel(null);
				//	iv_scope.getView().byId("firstLayerTable").getModel().refresh();
		iv_scope.getView().byId("secondLayerTable").setModel(null);
				//	iv_scope.getView().byId("secondLayerTable").getModel().refresh();
		iv_scope.getView().byId("thirdLayerTable").setModel(null);
				//	iv_scope.getView().byId("thirdLayerTable").getModel().refresh();
		iv_scope.getView().byId("fourthLayerTable").setModel(null);
				//	iv_scope.getView().byId("fourthLayerTable").getModel().refresh();
	...
	//    iv_scope.navBack();
        return;
				...
				// array filtreleyerek doldurma bince 17032021
				...
				//modele filtrelenmiş datayı atama bince 17032021

				if (iv_scope.idTamponListTable.length > 0) {
					var myModel = new JSONModel();
					myModel.setData(iv_scope.idTamponListTable);
					iv_scope
						.getView()
						.byId("idTamponListTable")
						.setModel(myModel);
					iv_scope
						.getView()
						.byId("idTamponListTable")
				...
				if (iv_scope.firstLayerTable.length > 0 || iv_scope.firstLayerTable.length == 0) {
					var myModel2 = new JSONModel();
					myModel2.setData(iv_scope.firstLayerTable);
					iv_scope
						.getView()
						.byId("firstLayerTable")
						.setModel(myModel2);
					iv_scope
						.getView()
						.byId("firstLayerTable")
				...
				if (iv_scope.secondLayerTable.length > 0 || iv_scope.secondLayerTable.length == 0) {
					var myModel3 = new JSONModel();
					myModel3.setData(iv_scope.secondLayerTable);
					iv_scope
						.getView()
						.byId("secondLayerTable")
						.setModel(myModel3);
					iv_scope
						.getView()
						.byId("secondLayerTable")
				...
				if (iv_scope.thirdLayerTable.length > 0 || iv_scope.thirdLayerTable.length == 0) {
					var myModel4 = new JSONModel();
					myModel4.setData(iv_scope.thirdLayerTable);
					iv_scope
						.getView()
						.byId("thirdLayerTable")
						.setModel(myModel4);
					iv_scope
						.getView()
						.byId("thirdLayerTable")
				...
				if (iv_scope.fourthLayerTable.length > 0 || iv_scope.fourthLayerTable.length == 0) {
					var myModel5 = new JSONModel();
					myModel5.setData(iv_scope.fourthLayerTable);
					iv_scope
						.getView()
						.byId("fourthLayerTable")
						.setModel(myModel5);
					iv_scope
						.getView()
						.byId("fourthLayerTable")
			...
			onDragStart: function (oEvent) {
				...
				this.draggedTable = oEvent.getParameter("target").getTable().sId.split("--")[1];
			...
			onDropTable2: function (oEvent) {
				this.droppedTable = oEvent.getParameter("droppedControl").sId.split("--")[1];

				if (this.droppedTable.includes("-") == true) {
					this.droppedTable = oEvent.getParameter("droppedControl").getTable().sId.split("--")[1]
				...
				if (this.droppedTable != "idTamponListTable") {
					...
					if (oEvent.getParameter("droppedControl").getModel() != undefined && oEvent.getParameter("droppedControl").getModel()?.getData()
						.length == 2) {
					...
					var draggedRowData = [];
					draggedRowData.push(oEvent.getParameter("dragSession").getComplexData("draggedRowContext").getModel()?.getObject(sPath));
					...
					var sBinding = table.getBindingPath("rows");
					var tableData = table.getModel()?.getObject(sBinding);
					var sIndex = oEvent.getParameter("dragSession").getComplexData("draggedRowContext").sPath.split("/")[1];
					var selectedObjects = [];
					...
					var oDragSession = oEvent.getParameter("dragSession");
					var oDraggedRowContext = oDragSession.getComplexData("draggedRowContext");
					var draggedData = oDragSession.getComplexData("draggedRowContext").getObject()
					var table = this.getView().byId(this.droppedTable);
					var sBinding = table.getBindingPath("rows");
					var tableData = table.getModel()?.getObject(sBinding);
		...
		hideBusyIndicator : function() {
			...
			onPressTamponConsumptionButton: function (oEvent) {			
				...
				var selectedLine = oEvent.getSource().getParent().getModel().getData()[sIndex];

				if (selectedLine.QTY_ON_HAND > 500) {
					MessageBox.show("500 kg ve üzerindeki bobinler için tampon tüket işlemi yapamazsınız");
					return;
            ...
            getCurrentStockFromInvId:function(selectedLine){            
                    ...
                    sap.ui.core.Fragment.byId("Z_StockChange", "tamponIdField").setText(inventoryId);
                ...
                getCurrentStockFromInvIdCB: function (iv_data, iv_scope) {
                    ...
                    if (iv_data[1] === "S") {
                        sap.ui.core.Fragment.byId("Z_StockChange", "quantityFieldOld").setValue(iv_data[0].Rowsets.Rowset.Row.QTY_ON_HAND*1000);
                ...
                onPresStockChangeSaveButton:function(){
                    ...
                    var response = TransactionCaller.async(
                        "ECZ_MES-4.0/KAGIT/DM_MANAGEMENT/COMBINE_ORDER/tamponManuelStockIncrease/T_MainTransactionManuelStockChange", {
                            I_INV: sap.ui.core.Fragment.byId("Z_StockChange", "tamponIdField").getText(),
                            I_SITE: this.SITE,
                            I_WC: this.selectedWorkCenter,
                            I_ENTRY_QNT_NEW: sap.ui.core.Fragment.byId("Z_StockChange", "quantityFieldNew").getValue(),
                            I_ENTRY_QNT_OLD: sap.ui.core.Fragment.byId("Z_StockChange", "quantityFieldOld").getValue()
                        },
                        "O_JSON",
			...
			setDragDropVisible : function() {
				...
				if (this.selectedOrderKatAdet == 3) {
					this.getView().byId("fourthLayerTable").setVisible(false);

				} else if (this.selectedOrderKatAdet == 2) {
					this.getView().byId("fourthLayerTable").setVisible(false);
					this.getView().byId("thirdLayerTable").setVisible(false);

				} else if (this.selectedOrderKatAdet == 1) {

					this.getView().byId("fourthLayerTable").setVisible(false);
					this.getView().byId("thirdLayerTable").setVisible(false);
					this.getView().byId("secondLayerTable").setVisible(false);

				} else if (this.selectedOrderKatAdet == 0) {
					this.getView().byId("fourthLayerTable").setVisible(false);
					this.getView().byId("thirdLayerTable").setVisible(false);
					this.getView().byId("secondLayerTable").setVisible(false);
					this.getView().byId("firstLayerTable").setVisible(false);
			...
			// visible yapılması istendiği için aşağısı kapatıldı 31032021 bince
			// setDragDropDisabled: function () {

			//     if (selectedOrderKatAdet == 3) {
			//         this.getView()
			//             .byId("fourthLayerTable")
			//             .getDragDropConfig()
			//             .forEach((index, input) => {
			...
			//     } else if (selectedOrderKatAdet == 2) {
			//         this.getView()
			//             .byId("fourthLayerTable")
			//             .getDragDropConfig()
			//             .forEach((index, input) => {
			...
			//     }
			//     else if (selectedOrderKatAdet == 1) {
			//         this.getView()
			//             .byId("fourthLayerTable")
			//             .getDragDropConfig()
			//             .forEach((index, input) => {
			...
			//     }
			//     else if (selectedOrderKatAdet == 0) {
			//         this.getView()
			//             .byId("fourthLayerTable")
			//             .getDragDropConfig()
			//             .forEach((index, input) => {
