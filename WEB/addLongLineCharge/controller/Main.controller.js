sap.ui.define(
	[
		...
		"sap/m/Button",
		"sap/m/ButtonType",
		"addLongLineCharge/scripts/transactionCaller",
	],
	function (
		...
		var myModel, SFC, RESOURCE, total, multi;
		return Controller.extend("addLongLineCharge.controller.Main", {
			onInit: function () {
			...
			getInitialDatasCB: function (iv_data, iv_scope) {
				...
				iv_scope.getView().byId("idUzunTable").setModel(myModel);
				iv_scope.getView().byId("idUzunTable").getModel().refresh();
				iv_scope.getTotal();
			...
			onPressSaveButton: function (oEvent) {
				...
				if (response[1] == "E") {
					...
					var jsonData = JSON.stringify(this.oView.byId('idUzunTable').getModel().getData().Rowsets.Rowset.Row);
					var response = TransactionCaller.sync("ECZ_MES-4.0/SELULOZ/longLineChargeScreen/T_SaveButton", {
							I_jsonData: jsonData,
							I_SFC: SFC,
							I_RESOURCE: RESOURCE,
						},
			...
			getTotal: function () {
				total = 0;
				var myModel = this.getView().byId("idUzunTable").getModel();
			...
			getMulti: function () {
				multi = 0;
				for (var i = 0; i < myModel.getData().Rowsets.Rowset.Row.length; i++) {
					multi += Number(myModel.getData().Rowsets.Rowset.Row[i].ADET * myModel.getData().Rowsets.Rowset.Row[i].WEIGHT1);
				}
				this.getView().byId("text3").setText(multi);
			},

			onSave: function () {
				var errorState = false;
				this.getView().byId("idUzunTable").getModel().getData().Rowsets.Rowset.Row.forEach((item) => {
				...
				if (total > 20) {
					...
					myModel.refresh();
					this.getView().byId("idUzunTable").setModel(myModel);
				}
			},
			...
			onPressAddSelulozButton: function (oEvent) {
				if (sap.ui.core.Fragment.byId("Z_SelulozAddFragment", "quantity").getValue() == 0 || sap.ui.core.Fragment.byId("Z_SelulozAddFragment", "quantity").getValue() == "" || sap.ui.core.Fragment.byId("Z_SelulozAddFragment", "idMaterialCombobox").getSelectedKey() == "") {
					...
					var weight = sap.ui.core.Fragment.byId("Z_SelulozAddFragment", "idMaterialCombobox").getModel().getData().Rowsets.Rowset.Row.find(
						(o) => o.ITEM == material).WEIGHT;
					...
					this.getView().byId("idUzunTable").getModel()?.getData()?.Rowsets.Rowset.Row.forEach((input, index) => {
					...
					allData.Rowsets.Rowset.Row[allData.Rowsets.Rowset.Row.length] = jsonDataforInputs2[0];
                    //var myModel = this.getView().byId("idUzunTable").getModel();
			...
			getAllMaterials: function () {
				TransactionCaller.async("ECZ_MES-4.0/KAGIT/selulozCombinationScreen/T_GetMaterialsBelongsToSeluloz", {
						I_TYPE: "Uzun"
					},
					"O_JSON",
			...
			getAllMaterialsCB: function (iv_data, iv_scope) {
				...
				sap.ui.core.Fragment.byId("Z_SelulozAddFragment", "idMaterialCombobox").setModel(myModel);
			},
			onPressSelulozAddButton: function () {
				if (!this._oDialogSelulozAdd) {
					this._oDialogSelulozAdd = sap.ui.xmlfragment("Z_SelulozAddFragment", "addLongLineCharge.view.fragments.addSeluloz", this);
			...
			onConfirmSelulozAddButton: function (oEvent) {
				...
				sap.ui.core.Fragment.byId("Z_SelulozAddFragment", "idSelulozAddTable").getModel()?.getData()?.forEach((input, index) => {
				...
				existingTableDataSeluloz[existingTableDataSeluloz.length] = jsonDataforInputs[0];
				var oModel = new sap.ui.model.json.JSONModel();

				oModel.setData(existingTableDataSeluloz);
				sap.ui.core.Fragment.byId("Z_SelulozAddFragment", "idUzunTable").setModel(oModel);
			},
			onCancelSelulozButton: function () {
				this._oDialogSelulozAdd.close();
			},
			onDelete: function () {
				var oTable = this.getView().byId("idUzunTable");
