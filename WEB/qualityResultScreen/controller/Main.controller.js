sap.ui.define(
	[
		...
		"sap/m/Button",
		"sap/m/ButtonType",
		'qualityResultScreen/scripts/transactionCaller'
	],
	function (
		Controller,
		JSONModel,
		...
		"use strict";
		var SFC, OPERATION, HANDLE, ROW_NUMBER, SAMPLE_TIME;

		return Controller.extend("qualityResultScreen.controller.Main", {
			onInit: function () {
			...
			openLogFragment: function () {
				if (SFC == null || SFC == "") {
					MessageBox.error("Sistemde ilgili SFC Numarası bulunamadı");
					return;
				}
				...
				var response = TransactionCaller.sync("ECZ_MES-4.0/QUALITY_RESULTS/checkParameters/T_CHECK_PARAMETERS", {
						I_SFC: SFC,
			...
			getDialog: function () {
				if (!this.oDialog) {
					this.oDialog = sap.ui.xmlfragment("logResults_Fragment", "qualityResultScreen.view.logResults_Fragment", this);
			...
			getTableData: function () {
				TransactionCaller.async("ECZ_MES-4.0/QUALITY_RESULTS/getParameters/T_GET_QUALITY_RESULTS", {
						I_SFC: SFC,
						I_OPERATION: OPERATION,
						I_HANDLE: HANDLE
					},
			...
			colorTableRows: function () {
				...
				for (var j = 0; j < length; j++) {
					...
					if (tableRowDetails[j].DATA_TYPE == "L") {
						...
						var maxValue = tableRowDetails[j].MAX_VALUE2;
						var maxValue2 = tableRowDetails[j].MAX_VALUE;
						var value = tableRowDetails[j].VALUE;
			...
			onPressSaveButton: function () {
				...
				var response = TransactionCaller.sync("ECZ_MES-4.0/QUALITY_RESULTS/sendParameters/T_SEND_PARAMETERS", {
