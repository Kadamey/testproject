sap.ui.define(
	[
		...
		"sap/m/Button",
		"sap/m/ButtonType",
		"addScraptLineCharge/scripts/transactionCaller",
	],
	function (
		Controller,
		JSONModel,
		...
		return Controller.extend("addScraptLineCharge.controller.Main", {
			onInit: function () {
			...
			getInitialDatas: function () {
				TransactionCaller.async(
					"ECZ_MES-4.0/SELULOZ/longLineChargeScreen/T_GetLongLineCharge_Dese", {
						I_SFC: SFC,
						I_TYPE: "Dese",
					},
					"O_JSON",
			...
			getInitialDatasCB: function (iv_data, iv_scope) {
				...
				iv_scope.getView().byId("idScraptTable").getModel().refresh();
				iv_scope.getTotal();
			...
			onPressSaveButton: function (oEvent) {
				...
				if (response[1] == "E") {
					...
					var jsonData = JSON.stringify(this.oView.byId("idScraptTable").getModel().getData().Rowsets.Rowset.Row);
					var response = TransactionCaller.sync(
						"ECZ_MES-4.0/SELULOZ/longLineChargeScreen/T_SaveButton_For_Dese", {
							I_jsonData: jsonData,
							I_SFC: SFC,
							I_RESOURCE: RESOURCE,
						},
			...
			getTotal: function () {
				total = 0;
				var myModel = this.getView().byId("idScraptTable").getModel();

				for (var i = 0; i < myModel.getData()?.Rowsets?.Rowset?.Row?.length; i++) {
					total += Number(myModel.getData()?.Rowsets?.Rowset?.Row[i]?.WEIGHT);
				}
			},

			onSave: function () {
				var errorState = false;
				this.getView().byId("idScraptTable").getModel().getData().Rowsets.Rowset.Row.forEach((item) => {
				...
				if (total > 1500) {
					...
					myModel.refresh();
					this.getView().byId("idScraptTable").setModel(myModel);
				}
			},
			...
			onPressAddSelulozButton: function (oEvent) {
				if (sap.ui.core.Fragment.byId("Z_SelulozAddFragment", "quantity").getValue() == 0 || sap.ui.core.Fragment.byId("Z_SelulozAddFragment", "quantity").getValue() == "" || sap.ui.core.Fragment.byId("Z_SelulozAddFragment", "idMaterialCombobox").getSelectedKey() == "") {
					...
					sap.ui.core.Fragment.byId(
						...
						"onPressAddSelulozButton").setEnabled(false);
					return;
				} else {
					...
					this.getView().byId("idScraptTable").getModel()?.getData()?.Rowsets.Rowset.Row.forEach((input, index) => {
			...
			onPressSelulozAddButton: function () {
				if (!this._oDialogSelulozAdd) {
					this._oDialogSelulozAdd = sap.ui.xmlfragment(
						"Z_SelulozAddFragment",
						"addScraptLineCharge.view.fragments.addSeluloz",
						this
					);
			...
			onDelete: function () {
				var oTable = this.getView().byId("idScraptTable");
