			...
			<soapenv:Body>
				<sch:ItemUpdateRequest_sync>
					<sch:Item>
						...
						<sch:Item>
							<xsl:choose>
								<xsl:when test="(string(format-number( ../E1MARA1/MATNR_EXTERNAL, '000000000000000000')))">
									<xsl:call-template name="addItem">
										<xsl:with-param name="item" select="format-number( ../E1MARA1/MATNR_EXTERNAL, '000000000000000000')"/>
									</xsl:call-template>
								</xsl:when>
								...
								<xsl:otherwise>
									<xsl:call-template name="addItem">
										<xsl:with-param name="item" select="format-number(../MATNR, '000000000000000000')"/>
									</xsl:call-template>
								</xsl:otherwise>
							</xsl:choose>
						</sch:Item>
